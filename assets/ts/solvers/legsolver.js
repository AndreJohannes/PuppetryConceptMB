...
(function (Solver) {
    ...
    Solver.Leg = Leg;
    var Leg_new = (function (_super) {
        __extends(Leg_new, _super);
        function Leg_new(offset) {
            _super.call(this, offset);
        }
        Leg_new.prototype.reimposeConstraints = function (state) {
            var index = this.offset;
            var x1 = state[index++];
            var y1 = state[index++];
            var z1 = state[index++];
        ...
        Leg_new.prototype.evaluate_free = function (state) {
            this.reimposeConstraints(state);
            var index = this.offset;
            var x1 = state[index++];
            var y1 = state[index++];
            ...
            var v12 = 2 * (Math.pow((z2dot - z1dot), 2) - fz * (z2 - z1) + Math.pow((y2dot - y1dot), 2) + (g2 - fy) * (y2 - y1) + Math.pow((x2dot - x1dot), 2) - fx * (x2 - x1));
        ...
        Leg_new.prototype.evaluate_constraint = function (state) {
            this.reimposeConstraints(state);
            var index = this.offset;
            var x1 = state[index++];
            var y1 = state[index++];
            ...
            var m32 = y1 - y2;
            var m33 = 1;
            var v11 = 2 * ((z4 - z1) * z4dotdot + Math.pow((z4dot - z1dot), 2) + (y4 - y1) * (y4dotdot - g) + Math.pow((y4dot - y1dot), 2) + (x4 - x1) * x4dotdot + Math.pow((x4dot - x1dot), 2));
            var v12 = 2 * (Math.pow((z2dot - z1dot), 2) + Math.pow((y2dot - y1dot), 2) + Math.pow((x2dot - x1dot), 2));
            var v13 = g;
            var denominator = m13 * m22 * m31 - m12 * m23 * m31
                - m13 * m21 * m32 + m11 * m23 * m32
                + m12 * m21 * m33 - m11 * m22 * m33;
        ...
        Leg_new.prototype.evaluate = function (state) {
            var y2 = state[this.offset + 7];
            if (y2 > -2000)
                return this.evaluate_free(state);
            var ret = this.evaluate_constraint(state);
            if (ret == null)
                return this.evaluate_free(state);
            return ret;
        };
        Leg_new.prototype.length = function () {
            return 18;
        };
        return Leg_new;
    })(Solver.Extremity);
    Solver.Leg_new = Leg_new;
})(Solver || (Solver = {}));
